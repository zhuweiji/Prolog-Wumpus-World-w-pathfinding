...
class KnownWorld:
    ...
    def query_compound_coords(self, compound):
        ...
        if not res or 'Output' not in res: raise KBSPrinterException(
            f'Could not query values of {compound} coordinates')
...
class MapCell:
    ...
    def confounded_repr(self):
        return '%' if self.confounded else '.'

    def stench_repr(self):
        return '=' if self.stench else '.'

    def tingle_repr(self):
        return 'T' if self.tingle else '.'

    def NPC_repr(self):
        return '-' if self.portal or self.wumpus else ' '

    def glitter_repr(self):
        return '*' if self.glitter else '.'

    def bump_repr(self):
        return 'B' if self.bump else '.'

    def scream_repr(self):
        return '@' if self.scream else '.'

    def middle_cell(self):
        if self.portal and self.wumpus:
            return 'U'
        elif self.wumpus:
            return 'W'
        elif self.portal:
