    ...
    (
        ...
        , (
            ...
            ;(
                ...
                % , Action=moveforward
                simPerformAction([X0,Y0,D0],[X2,Y2,D2], Action)
                , bexplore([X2,Y2,D2], ListOfActions, I1)
            )
        )
    )
    .
...
simPerformAction([X0,Y0,D0],[X1,Y1,D1], Action) :-
    (
        (Action=moveforward , moveforward([X0,Y0,D0],[X1,Y1, D1]))
